...
def of_devil(devil_audio,devil_ly,devil_image=None,font="#ffffff",back="#ffffff"):
    ...
    audio,x,devil_count,y,devil_,angel_ly,counter=MP3(devil_audio),80,0,119,[],lyrics(devil_ly),0
    devil_time=int(audio.info.length)
    print(devil_time)
    clip,devil=VideoFileClip("video.mp4"),cv2.resize(rect_with_rounded_corners(cv2.cvtColor(cv2.imread(devil_image),cv2.COLOR_BGR2RGB),10,1,hex_to_rgb(back)),(500,500)) 
    ...
    devil=CompositeVideoClip([video_]+devil_).set_audio(AudioFileClip(devil_audio))
...
def div():
    ...
    if request.method == "POST":
        ...
        angel_ly=lyrics(f"{os.listdir('devil/lyrics')[0]}")
        (AudioSegment.from_mp3(f"devil/song/{os.listdir('devil/song')[0]}")[(angel_ly[0][0]*1000):(angel_ly[len(angel_ly)-1][0]*1000)]).export(f"devil/song/{os.listdir('devil/song')[0]}", format="mp3")
